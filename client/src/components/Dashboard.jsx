...
const Dashboard = ({data,fetchData}) => {
  ...
  const handleBinClick = (group_id, wrack_id, bin_id) => {
    axios.get('http://localhost:5000/bin', {
      params: { group_id, wrack_id, bin_id }
    })
      .then(response => {
  ...
  const toggleSchedule = (group_id, wrack_id, bin_id, index, current_status) => {
    axios.put('http://localhost:5000/bin/update/schedule', {
      group_id,
      wrack_id,
      bin_id,
  ...
  const updateColor = (group_id, wrack_id, bin_id, new_color) => {
    axios.put('http://localhost:5000/bin/update/color', {
      group_id,
      wrack_id,
      bin_id,
      new_color
